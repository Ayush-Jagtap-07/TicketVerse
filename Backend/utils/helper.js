...
const fs = require('fs');
const getStream = require('get-stream'); // npm i get-stream
...
async function generateTicketPDF(ticketDetails, type) {
    const doc = new PDFDocument({ margin: 50 });
...
async function sendTicketEmail(email, name, pdfBuffer, type) {
    try {
        console.log("📩 sendTicketEmail called with:", email);
        let transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
                user: process.env.TV_EMAIL,
